...
class ServicesController extends AppController
{
    public function beforeFilter(\Cake\Event\Event $event)
    {
        ...
        $this->Auth->allow(['searchService', 'modifyColumns']);
    }
    ...
    public function isAuthorized($user)
    {
        if(isset($user['role']))
        {
            ...
            {
                if(in_array($this->request->action, ['add', 'edit', 'index', 'findServiceCost', 'specialIndex', 'updateRate', 'discountSurcharge']))
                {
                    return true;
                }
            }
            ...
            {
                if(in_array($this->request->action, ['index', 'findServiceCost', 'updateRate', 'discountSurcharge']))
                {
                    return true;
                }
            }			
    ...
    public function index()
    {
        ...
        {			
			foreach ($_POST['service'] as $servicePost)
            {
                ...
                if (substr($servicePost['service_code'], -3, 1) == ',')
                {
                    $replace1= str_replace('.', '', $servicePost['service_code']);
                    $replace2 = str_replace(',', '.', $replace1);
                    $service->service_code = $replace2;
                }
                else
                {
                    $service->service_code = $servicePost['service_code'];
                }
				...
				$tasaDolar = $system->extra_column1;

				$service->cost_bolivars = $service->service_code * $tasaDolar;
		...
		$tasaDolarActual = $system->extra_column1;		
		
        $this->set(compact('services', 'currentView', 'tasaDolarActual'));
        $this->set('_serialize', ['services', 'currentView', 'tasaDolarActual']);
    }
    ...
    public function add($controller = null, $action = null)
    {
        ...
        if ($this->request->is('post')) 
        {
            ...
            if (substr($_POST['service_code'], -3, 1) == ',')
            {
                $replace1= str_replace('.', '', $_POST['service_code']);
                $replace2 = str_replace(',', '.', $replace1);
                $service->service_code = $replace2;
            }
    ...
    public function updateRate($tasaDolar)
    {			
		$this->autoRender = false;
		...
		if (isset($_POST["tasaDolar"]))
		{
			...
			$system->extra_column1 = $dollarRate;
			...
			if ($this->Systems->save($system)) 
			{
				...
				if ($arrayResult['indicator'] == 0)
				{
					...
					{					
						$service->cost_bolivars = $service->service_code * $dollarRate;
						
						if (!($this->Services->save($service))) 
						{
							$errorIndicator = 1;
							$this->Flash->error(__('Error actualizando el servicio ID: ' . $service->id));						
							break;
						}
					}
					if ($errorIndicator == 0)
					{
						$this->Flash->success(__('La tarifa se actualizó satisfactoriamente'));
					}
				}
				else
				{
					$this->Flash->error(__('No se pudo actualizar la tarifa. No se encontraron servicios'));						
				}
			}
			else
			{
				$this->Flash->error(__('No se pudo actualizar la tarifa'));
			}
		}
		else
		{
    ...
    public function discountSurcharge($descuentoRecargo)
    {			
		$this->autoRender = false;
		...
		{			
			...
			if ($discountSurcharge < 0)
			{
				...
				$discountSurcharge = ($discountSurcharge * -1)/100;
			}
			else
			{
				$operationType = "Recargo";
				$discountSurcharge = $discountSurcharge/100;
			}
			...
			$dollarRate = $system->extra_column1;
			...
			if ($arrayResult['indicator'] == 0)
			{
				...
				foreach ($services as $service)
				{
					$discountAmountSurcharge = $service->service_code * $discountSurcharge;
					
					if ($operationType == "Descuento")
					{
						$service->service_code = $service->service_code - $discountAmountSurcharge;
					}
					else
					{
						$service->service_code = $service->service_code + $discountAmountSurcharge;
				...
				if ($errorIndicator == 0)
				{
					$this->Flash->success(__('El descuento/recargo se aplicó satisfactoriamente'));
				}
			}
			else
			{
				$this->Flash->error(__('No se pudo aplicar el descuent/recargo. No se encontraron servicios'));	
			}
		}
		else
		{
    ...
    {			
		...
		if ($arrayResult['indicator'] == 0)
		{
			...
			foreach ($services as $service)
			{
				$serviceGet = $this->Services->get($service->id);
				
				$serviceGet->service_code = $service->cost_bolivars / $dollarRate;
							
				if (!($this->Services->save($serviceGet)))
				{
					$errorIndicator = 1;
					echo "<p>Error al hacer la conversión de precio del servicio ID: " . $service->id . "</p>";
					break;
				}
			}
			if ($errorIndicator == 0)
			{
				echo "<p>La conversión de los precios se realizó exitosamente</p>";		
			}
		}
		else
		{
			echo "<p>No se pudo hacer la conversión de los precios</p>";
		}
    }
}